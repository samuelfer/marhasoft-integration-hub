package br.com.marhasoft.integrationhub.modules.contabilidade.receitaorcamentaria.model;

import br.com.marhasoft.integrationhub.core.model.BatchRequest;
import jakarta.validation.Valid;
import jakarta.validation.constraints.NotEmpty;
import lombok.Builder;
import lombok.Getter;

import java.util.List;

@Getter
@Builder
public class ReceitaOrcamentariaBatchRequest implements BatchRequest<ReceitaOrcamentariaRequest> {

    @NotEmpty(message = "É necessário informar pelo menos uma receita orçamentária.")
    @Valid
    private List<ReceitaOrcamentariaRequest> elementos;

    private Boolean processamentoAssincrono;

    private String identificadorLote;

    private String observacao;

}